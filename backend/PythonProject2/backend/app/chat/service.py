...
def _find_latest_generated_image(
    db: Session,
    session_id: int
) -> Optional[str]:
    """
    ...
    try:
        latest_msg = (
            ...
            .filter(
                ...
                ChatMessage.generated_images.isnot(None)  # 有生成图片的消息
            )
            .order_by(ChatMessage.created_at.desc(), ChatMessage.id.desc())
            .first()
        )
        
        if latest_msg and latest_msg.generated_images:
            # 返回第一张生成的图片URL
            image_url = latest_msg.generated_images[0] if isinstance(latest_msg.generated_images, list) else None
            if image_url:
                logger.debug(f"[Chat Service] [IMAGE] 找到最近生成的图片: {image_url[:100]}...")
                return image_url
        
        logger.debug(f"[Chat Service] [IMAGE] 未找到最近生成的图片")
        return None
    except Exception as e:
        logger.warning(f"[Chat Service] [IMAGE] 查找最近生成图片时出错: {e}")
        return None
    ...
    try:
        ...
        if intent == IntentType.IMAGE_GENERATE:
            ...
            if images and len(images) > 0:
                ...
                # 查找最近生成的图片
                source_image = _find_latest_generated_image(db, session.id)
                
                if source_image:
                    if should_use_prev:
                        logger.info(f"[Chat Service] [IMAGE] ✅ 用户提到使用上一张图，找到最近生成的图片，将用于以图生图")
                    else:
                        ...
                        if has_modify_keyword:
                            logger.info(f"[Chat Service] [IMAGE] ✅ 用户消息包含修改关键词，找到最近生成的图片，将用于以图生图")
                        else:
                            logger.debug(f"[Chat Service] [IMAGE] 找到最近生成的图片，但用户未明确要求使用，将使用纯文本生成")
                            source_image = None  # 不使用上一张图，使用纯文本生成
                else:
                    if should_use_prev:
                        logger.warning(f"[Chat Service] [IMAGE] ⚠️ 用户提到使用上一张图，但未找到最近生成的图片，将使用纯文本生成")
                    else:
                        logger.debug(f"[Chat Service] [IMAGE] 用户未上传图片且未找到历史生成图片，使用纯文本生成")
