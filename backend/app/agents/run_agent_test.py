# backend/app/agents/run_agent_test.py
"""
Agent模块测试文件（可直接运行）
...
使用方法：
1. 直接运行：python agents/run_agent_test.py
2. 或在IDE中直接运行此文件（右键 -> Run）

这是一个独立的测试脚本，不是pytest测试文件
"""
...
# 添加backend目录到路径（使backend.app导入正常工作）
# 文件位置: E:\PythonProject2\backend\app\agents\run_agent_test.py
# 需要添加: E:\PythonProject2\backend 到路径
